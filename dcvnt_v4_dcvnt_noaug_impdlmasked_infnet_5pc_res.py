...
class CTDataset(data.Dataset):
    def __init__(self, data_home="",
        ...
        meta_pos   = [[os.path.join(data_home, "NpyData-size224x336-infmask1010-5pc", "{}.npy".format(x)), 1]
                                for x in readvdnames(_embo_f)]

        meta_neg   = [[os.path.join(data_home, "NpyData-size224x336-infmask1010-5pc", "{}.npy".format(x)), 0]
                                for x in readvdnames(_norm_f)]

        meta_cap   = [[os.path.join(data_home, "NpyData-size224x336-infmask1010-5pc", "{}.npy".format(x)), 2]
...
model.load_state_dict(torch.load('experiments_v4_dcvnt_noaug_infmask1010_5pc/ncov-Epoch_00015.pth'))
model.eval()
# model.module.classifier[1] = nn.Linear(model.module.classifier[1].in_features, NUM_CLASSES).cuda()
...
WEIGHT_DECAY = 0
LR_DECAY = 1
INIT_MODEL_PATH = 'experiments_v4_dcvnt_noaug_infmask1010_5pc/ncov-Epoch_00015.pth'
INIT_MODEL_STRICT = "True"
SNAPSHOT_FREQ = 5
TRAIN_EPOCH = 200 #, will likely stop it early
SNAPSHOT_HOME = "experiments_v4_dcvnt_noaug_infmask1010_5pc_15ep"
