...
class CTDataset(data.Dataset):
    def __init__(self, data_home="",
        ...
        _embo_f = os.path.join(data_home, "ImageSets", "n_ncov_{}.txt".format(split))
        _norm_f = os.path.join(data_home, "ImageSets", "n_normal_{}.txt".format(split))
        _cap_f = os.path.join(data_home, "ImageSets", "n_cap_{}.txt".format(split))
        # Build a dictionary to record {path - label} pair
        meta_pos   = [[os.path.join(data_home, "NpyData-size224x336-impdlmask", "{}.npy".format(x)), 1]
                                for x in readvdnames(_embo_f)]

        meta_neg   = [[os.path.join(data_home, "NpyData-size224x336-impdlmask", "{}.npy".format(x)), 0]
                                for x in readvdnames(_norm_f)]

        meta_cap   = [[os.path.join(data_home, "NpyData-size224x336-impdlmask", "{}.npy".format(x)), 2]
...
SNAPSHOT_HOME = "experiments_v4_dcvnt_impdlmask_ND"
