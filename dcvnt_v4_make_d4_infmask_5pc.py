...
save_path = 'dataset4/NCOV-BF/NpyData-infmask-5pc/'
model = Inf_Net()
...
for i in range(test_loader.size):
    ...
    for i, img in enumerate(images): # ([64, 1, 3, 352, 352])
        ...
        res = (np.ceil(res-0.05)*255).astype(np.uint8) # rounds up everything above 5% chance of infection
...
des_home = 'dataset4/NCOV-BF/NpyData-infmask-5pc' # '/content/test'
...
def create_masked_lungs(x):
    ...
    raw_infmasked00 = np.zeros((length, 512, 512))
    ...
    for i in range(length):
        ...
        raw_infmasked00[i] = cv2.bitwise_and(raw_imgs[i], raw_imgs[i], mask=dilated_slice)

    np.save(os.path.join(des_home, x+"-infmask00.npy"), raw_infmasked00) # this is the non dilated version
...
src_home = 'dataset4/NCOV-BF/NpyData-infmask-5pc'
des_home = 'dataset4/NCOV-BF/NpyData-size224x336-infmask1010-5pc'
