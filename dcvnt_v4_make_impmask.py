...
# where saving
des_home = 'dataset4/NCOV-BF/NpyData-size224x336-impdlmask'

# where loading from
src_home = 'dataset4/NCOV-BF/NpyData-size224x336'

def imp_masks(x):        # dtype is "PE"/"NORMAL"
    print (x)
    ...
    np.save(os.path.join(des_home, x+"-dlmask.npy"), imp_masks)
...
with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
    fs = [executor.submit(imp_masks, x, ) for x in pe_list[::-1]]
