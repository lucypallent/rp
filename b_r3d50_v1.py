# generate r3d50 based on here https://github.com/kenshohara/3D-ResNets-PyTorch/blob/master/models/resnet.py

# train two classifiers one to see if a patient has pneumonia (Normal vs CAP & COVID)
# the other to see is a patient has CAP or COVID
...
r3d50_1 = generate_model(50, n_classes=700)
PATH = 'r3d50_K_200ep.pth'
# PATH = '/content/drive/MyDrive/curated_data/r3d50_K_200ep.pth'

# Model class must be defined somewhere
r3d50_1.load_state_dict(torch.load(PATH)['state_dict'])
r3d50_1.eval()
r3d50_1 = r3d50_1.to(device)

# so we're doing feature extraction
for p in r3d50_1.parameters():
    p.requires_grad = False

# change to classify for 2 classes
r3d50_1.fc = nn.Linear(r3d50_1.fc.in_features, 2).to(device) # 2 classes
...
optimizer = optim.SGD(r3d50_1.parameters(), lr=0.001, momentum=0.9)
...
target_names = ['NonCOVID', 'Pneumonia']

print('Model outputs for b_r3d50_1:')
...
torch.save(r3d50_1.state_dict(), 'b_r3d50_1.pth')
...
ctTrainDataset_2 = ctDataset2_2(df=all_df_train, root_dir='~/rp/dataset2/dataset2')
# ctTestDataset_2 = ctDataset2_2(df=all_df_test, root_dir='/content/drive/MyDrive/curated_data/curated_data')
ctTestDataset_2 = ctDataset2_2(df=all_df_test, root_dir='~/rp/dataset2/dataset2')
...
r3d50_2 = generate_model(50, n_classes=700)
PATH = 'r3d50_K_200ep.pth'
# PATH = '/content/drive/MyDrive/curated_data/r3d50_K_200ep.pth'

# Model class must be defined somewhere
r3d50_2.load_state_dict(torch.load(PATH)['state_dict'])
r3d50_2.eval()
r3d50_2 = r3d50_2.to(device)

# so we're doing feature extraction
for p in r3d50_2.parameters():
    p.requires_grad = False

# change to classify for 2 classes
r3d50_2.fc = nn.Linear(r3d50_2.fc.in_features, 2).to(device) # 2 classes
...
optimizer = optim.SGD(r3d50_2.parameters(), lr=0.001, momentum=0.9)

for epoch in range(NUM_EPOCHS):
    for i, data in enumerate(ctTrainDataloader_2):
        images = data['images'].to(device)
        labels = data['labels'].to(device)
        optimizer.zero_grad()
        outputs = r3d50_2(images)
        loss = F.cross_entropy(outputs, labels)
        loss.backward()
        optimizer.step()
...
pred = torch.tensor([]).to(device)
 ...
 # test for t
for i, data in enumerate(ctTestDataloader_2):
...
target_names = ['COVID', 'Pneumonia']

print('Model outputs for b_r3d50_2:')
...
# Colab path commented out
# torch.save(r3d50_2.state_dict(), 'drive/MyDrive/curated_data/baseline_r3d50_v0_test_2epoch.pth')
torch.save(r3d50_2.state_dict(), 'b_r3d50_2.pth')


# final output combining the two

# add back in the initial class
# CHANGE HERE FOR url in io.imread
# creating a class to read in this second dataset
# now adding in the custom dataset
...
ctTestDataloader=torch.utils.data.DataLoader(ctTestDataset, batch_size=BATCH_SIZE, num_workers=0)
...
print('Model outputs for b_r3d50_T:')
