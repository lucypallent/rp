...
class CTDataset(data.Dataset):
    def __init__(self, data_home="",
        ...
        meta_pneum   = [[os.path.join(data_home, "NpyData-size224x336-infmask1010-test-1pc", "{}.npy".format(x)), 1]
                                for x in readvdnames(_pneum_f)]

        meta_non_pneum   = [[os.path.join(data_home, "NpyData-size224x336-infmask1010-test-1pc", "{}.npy".format(x)), 0]
...
model.eval() # eval becauuse evaluating here not training
...
for param in model.parameters():
    param.requires_grad = False
model.module.classifier[1] = nn.Linear(model.module.classifier[1].in_features, NUM_CLASSES).cuda()
model.train() # train because training here
        ...
        if e % SNAPSHOT_FREQ == 0 or e >= TRAIN_EPOCH-1:
            ...
            with torch.no_grad():
                ...
                for i, (all_F, all_L, all_info) in enumerate(ValidLoader):
                    ...
                    if labels[0] == 0:
                         run["validation/0CAP/labels/loss"].log(val_loss)
                         run["validation/0CAP/labels/acc"].log(val_acc)
                         Val_Acc_non_pneum.write(val_acc)
                    elif labels[0] == 1:
                         run["validation/1COVID19/labels/loss"].log(val_loss)
                         run["validation/1COVID19/labels/acc"].log(val_acc)
                         Val_Acc_pneum.write(val_acc)

                    prob_preds = F.softmax(preds, dim=1)
                    if prob_preds[0, 0] > prob_preds[0, 1]: # ie prob of 0 NonPneum is higher
                        run["validation/0CAP/pred/loss"].log(val_loss)
                        run["validation/0CAP/pred/acc"].log(val_acc)
                    elif prob_preds[0, 0] < prob_preds[0, 1]: # ie prob of 1 Pneum is higher
                        run["validation/1COVID19/pred/loss"].log(val_loss)
                        run["validation/1COVID19/pred/acc"].log(val_acc)
                ...
                if Eacc > best_acc:
                    ...
                    run["validation/0CAP/bestValAcc"].log(Eacc_np)
                    run["validation/1COVID19/bestValAcc"].log(Eacc_p)
