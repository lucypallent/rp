##### THIS IS THE [masked lungs, old dataset]
##### on slurm as msk
### just putting in the masked lungs
...
#                     ANGLE_R=10, TRANS_R=0.1,
#                     SCALE_R=0.2, SHEAR_R=10,
#                     BRIGHT_R=0.5, CONTRAST_R=0.3): --- comment out masks
    # To Image.Image instances
    pil_imgs = [Image.fromarray((x * 255).astype(np.uint8)) for x in imgs]
    # pil_masks = [Image.fromarray((x * 255).astype(np.uint8)) for x in masks] --- comment out masks
    w, h = pil_imgs[0].size
    ...
    def affop(img, angle, translate, scale, shear, hflp, vflp, deformation):
        ...
        # if hflp: ---- aug seemed to be ruining training
        #     _img  = TF.hflip(_img) ---- aug seemed to be ruining training
        # if vflp: ---- aug seemed to be ruining training
        #     _img  = TF.vflip(_img) ---- aug seemed to be ruining training

        # _img = np.asarray(_img, dtype=np.uint8) ---- aug seemed to be ruining training
        # _img = elasticdeform.deform_grid(_img, deformation) ---- aug seemed to be ruining training
        # _img = Image.fromarray((_img * 255).astype(np.uint8)) ---- aug seemed to be ruining training
    ...
    # pil_masks = [affop(x, angle, translate, scale, shear, hflp, vflp, deformation) for x in pil_masks] -- comment out masks

    # Color Transforms
    def colorop(img, bright, contrast, gamma):
        _img = TF.adjust_brightness(img, bright)
        _img = TF.adjust_contrast(_img, contrast)
        # _img = TF.adjust_gamma(_img, gamma) ---- aug seemed to be ruining training
        return _img
    ...
    # noise Transforms ---- aug seemed to be ruining training (down to below)
    # def noisop(img, gaus):
    ...
    # pil_imgs = [noisop(x, gaus_noise) for x in pil_imgs] ---- aug seemed to be ruining training (up to above)

    imgs = np.asarray([np.asarray(x, dtype=np.uint8) for x in pil_imgs], dtype=np.uint8)
    # masks = np.asarray([np.asarray(x, dtype=np.uint8) for x in pil_masks], dtype=np.uint8) --- comment out masks
    return imgs#, masks --- comment out masks
...
class CTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # mask_path = data_path.replace('-masked.npy', '-inf10masked10k.npy') comment out mask

        cta_images = np.load(data_path)
        # cta_masks = np.load(mask_path) -- comment out mask
        ...
        if self.split == "train":
            ...
            #                                  BRIGHT_R=0.5, CONTRAST_R=0.3) - comment out mask
        ...
        # images = np.concatenate([cta_images[None, :, :, :], cta_masks[None, :, :, :]], axis=0) - comment out mask
...
SNAPSHOT_FREQ = 5
TRAIN_EPOCH = 2#00 #, will likely stop it early
SNAPSHOT_HOME = "experiments_v5_msk"
...
data_tfms = {'function': 'cta_images = Rand_Transforms(cta_images, ANGLE_R=10, TRANS_R=0.1, SCALE_R=0.2, SHEAR_R=10, BRIGHT_R=0.5, CONTRAST_R=0.3)',
'ANGLE_R': 10,
'TRANS_R': 0.1,
'SCALE_R': 0.2,
'SHEAR_R': 10,
